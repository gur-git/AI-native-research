...
**Production is at least two operations, not one, and they respond to AI differently.** Producing a literature summary, a routine analysis, or a draft of a familiar argument is *recombination* of existing material — what AI does well, where the cost-structure inversion described in §2 mostly holds. Producing a new framing, a method that did not exist, a hypothesis nobody has stated is *innovation* — and the literature on innovation, going back to Campbell's BVSR theory, is unanimous that innovation requires variation that is *not* directed by the goal, combined with retrospective selection. AI is structurally a parallel exploitation engine more than a parallel exploration engine. Its cost reduction applies cleanly to recombination and does not apply cleanly to innovation. A methodology that does not distinguish the two will be correct about routine work and silently wrong about the part of research that matters most. The deeper treatment is in [`02-innovation.md`](02-innovation.md).
...
The structure: the diagnosis lives in [`foundation/`](.) — this document, [`01-ai-attributes.md`](01-ai-attributes.md) (the catalog), [`02-innovation.md`](02-innovation.md) (the deep-dive on innovation as a different operation), [`03-implications.md`](03-implications.md) (gestures toward methodology), [`04-open-questions.md`](04-open-questions.md) (what is still open). The methodology lives in [`../methodology/guidelines.md`](../methodology/guidelines.md), grows from evidence, and starts with the initial guidelines drawn from this foundation. The evidence lives in [`../evidence/`](../evidence/). The chronicle of how the inquiry has changed lives in [`../log/`](../log/). Real research projects, when applied, will live in [`../case-studies/`](../case-studies/).
...
*Last revised 2026-05-06. Substantial rewrite per the §1–§6 structure, with empower-not-replace established as the central mindset and the structural skill-substitution claim as the load-bearing diagnostic. Supersedes the 2026-05-05 draft, which is preserved in `log/` for the record.*
