...
Foundation v1 as of 2026-05-06: five documents (`00`–`04`) covering diagnosis, attributes catalog, innovation deep-dive, implications, and open questions. The methodology has 15 guidelines, all at status `proposed`, plus a speculative `v0` synthesis of the end-to-end process ([`methodology/ai-native-research-process.md`](methodology/ai-native-research-process.md)) — explicitly a hypothesis to test, not evidence-backed guidance. Evidence collection has begun beyond the reading list: a self-study stream ([`evidence/self-study/`](evidence/self-study/)) and a lab survey ([`evidence/lab-surveys/`](evidence/lab-surveys/), awaiting data).

The repo is entering its first user-facing phase. Its first pilot — and first user — is a real research project (the maintainer's own), against which the methodology is applied and from which the repo learns. The immediate work is the general tooling that lets a researcher run AI-native research with their own AI agent: a handoff file that scaffolds a research workspace, and a README that orients a researcher to the repo. No guideline status has changed on the strength of the pilot yet; that waits for its evidence.
